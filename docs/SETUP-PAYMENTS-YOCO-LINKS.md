...
2. On **Profile** or the **Application** page, tap **Pay R60 securely** (or R40).
...
3. In **Admin** → select the student → **Application fee** → click **Mark R60 paid** (or R40).
...
2. Create a link for **R60** — copy the URL.
3. Optional: create a second link for **R40** (split payments).
...
| Variable | Example |
|----------|---------|
| `VITE_YOCO_PAYMENT_LINK_R95` | `https://…` (your **R60** link; env name unchanged) |
| `VITE_YOCO_PAYMENT_LINK_R50` | `https://…` (optional **R40** link; falls back to R60 link if empty) |
...
2. Tap **Pay R60 securely** — Yoco page should open.
3. Pay with a real or test card (per Yoco’s link settings).
4. In **Admin**, mark that student **Mark R60 paid**.
